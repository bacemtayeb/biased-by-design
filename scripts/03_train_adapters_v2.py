#!/usr/bin/env python3
"""
03_train_adapters_v2.py  — Upgraded LoRA config (v2)

Changes from v1:
  - r=32, lora_alpha=64 (2x capacity)
  - Target modules: q/k/v/o + gate/up/down proj (7 modules, covers MLP)
  - 4 epochs, early_stopping_patience=5
  - learning_rate=1e-4 (more stable at r=32)
  - Output: adapters_v2/{community}/

Usage:
    python3 scripts/03_train_adapters_v2.py --community politics
"""
...
ADAPTER_DIR = BASE_DIR / "adapters_v2"
...
def safe_batch_config():
    ...
    if vram > 0 and vram < 15_000:
        ...
        print("  Config: batch=16, accum=1 (H100/H200)")
        return 16, 1
    elif vram >= 40_000:
        print("  Config: batch=8, accum=2 (A100)")
        return 8, 2
    else:
...
def train(community: str, reset: bool = False):
    ...
    print(f"  Experiment: 2 — upgraded LoRA (r=32, 7 modules, 4 epochs)")
    print(f"{'='*60}")
